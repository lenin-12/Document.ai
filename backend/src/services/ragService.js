 ...
 * This ensures that duplicate context is not sent to the LLM (as per Requirement 14).
 * @param {Array} docs - Array of retrieved LangChain documents
...
const deduplicateDocuments = (docs) => {
  ...
  return docs.filter((doc) => {
    // Generate a unique key for each document chunk based on its content, page, and chunk index
...
export const processDocumentAndStore = async (docId, chunks, pages, filename) => {
  ...
  if(isOpenAiKeyValid()){
    ...
    // Index chunks in the BM25 index for keyword search (Requirement 6)
...
export const answerQuestion = async (docId, question) => {
  ...
  if (isOpenAiKeyValid()) {
    ...
    // Initialize the LLM to generate multiple search queries and perform RAG
    const llm = new ChatOpenAI({
      openAIApiKey: process.env.OPENAI_API_KEY,
      modelName: "gpt-4o-mini",
      temperature: 0.1,
    ...
    // Create the custom HybridRetriever combining Vector and BM25 searches (Requirement 5)
    ...
    // Initialize MultiQueryRetriever using the existing LLM and hybrid retriever
    const multiQueryRetriever = MultiQueryRetriever.fromLLM({
      retriever: hybridRetriever,
      llm: llm,
    });

    // Intercept and log the queries generated by the LLM for easy debugging
    ...
    // Retrieve documents using multi-query generation (LLM generates 3 variations by default)
    const rawDocs = await multiQueryRetriever.invoke(question);

    // Deduplicate the combined retrieved documents before sending context to the LLM (Requirement 14)
...
export const answerQuestionStream = async (docId, question, { onMetadata, onToken, onEnd, onError }) => {
  try {
    ...
    if (isOpenAiKeyValid()) {
      ...
      // Initialize a non-streaming LLM for MultiQueryRetriever query generation
      ...
      // Create the base retriever from vector store (retrieving top 4 chunks per query)
      // Create the custom HybridRetriever combining Vector and BM25 searches (Requirement 5)
      const hybridRetriever = new HybridRetriever({
        docId: docId,
        vectorStore: vectorStore,
      ...
      // Initialize MultiQueryRetriever using the LLM and hybrid retriever
      const multiQueryRetriever = MultiQueryRetriever.fromLLM({
        retriever: hybridRetriever,
